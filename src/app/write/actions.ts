 ...
 */
export async function generateSummaryAction(slug: string, body?: string): Promise<GenerateSummaryResult> {
  "use server";
  const user = await getCurrentUser();
  ...
  const now = new Date();
  await aiStore.enqueueSummary(handle, slug, now);
  ...
  const done = await aiStore.getTask(handle, slug);
